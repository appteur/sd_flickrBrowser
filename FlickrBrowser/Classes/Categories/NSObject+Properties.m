...
+ (Class) expectedValueTypeForPropertyNamed:(NSString*)name
{
    return NSClassFromString([self expectedValueTypeAsStringForPropertyNamed:name]);
}

+ (NSString*) expectedValueTypeAsStringForPropertyNamed:(NSString*)name
{
    NSString *fullTypeStr   = [NSString stringWithUTF8String:[self typeOfPropertyNamed:name]];
    NSInteger length        = fullTypeStr.length - 4;
    NSRange classRange      = {3,length};
    NSString *typeOnly      = [fullTypeStr substringWithRange:classRange];
    //    NSLog(@"Property: [%@] Expecting type: [%@]", name, typeOnly);
...
+ (NSArray *) propertyNames
{
	...
	for ( i = 0; i < count; i++ )
    {
        NSString *string = [NSString stringWithUTF8String: property_getName(properties[i])];
